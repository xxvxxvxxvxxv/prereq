...
(function () {
  ...
  function render(reset=false){
    ...
    const activity=state.indexMeta?.error?' · check paused':state.indexMeta?.currentCourse?` · reading ${state.indexMeta.currentCourse}`:state.indexMeta?.refreshing?' · queued':'';
    const reviewed=state.indexMeta?.reviewRequired?` · ${state.indexMeta.reviewRequired} need review`:'';
    $('match-count').textContent=`${cv.checked}/${cv.total} prerequisites checked${reviewed}${activity}${state.layout.capped?' · view limit':''}`;
    $('match-count').title=state.indexMeta?.error||state.indexMeta?.lastError||'Rules are read course by course; open this counter for progress and source errors.';
    renderGraph();
